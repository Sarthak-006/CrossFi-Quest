...
# Production-grade app configuration with enhanced styling
st.set_page_config(
...
# Enhanced CSS for professional educational app design
st.markdown("""
<style>
    /* Import Google Fonts */
    ...
    /* Card styling */
    .lesson-card, .quiz-card, .wallet-card {
        background: white;
    ...
    .lesson-card:hover, .quiz-card:hover, .wallet-card:hover {
...
def render_wallet_connection():
    ...
    if not st.session_state.wallet['connected']:
        ...
        with st.expander("📋 CrossFi Testnet Configuration", expanded=False):
            ...
            with config_col1:
                st.code(f"""Network Name: {CROSSFI_TESTNET_CONFIG['chainName']}
Chain ID: {CROSSFI_TESTNET_CONFIG['chainId']}
Symbol: {CROSSFI_TESTNET_CONFIG['nativeCurrency']['symbol']}
                """)
            with config_col2:
                st.code(f"""RPC URL: {CROSSFI_TESTNET_CONFIG['rpcUrls'][0]}
...
def render_user_profile():
    ...
    if current_level < 50:
        ...
        xp_requirements = [0, 200, 500, 1000, 1800, 3000]
        
        # Calculate next level XP requirement
        if current_level <= 5:
            next_level_xp_req = xp_requirements[min(current_level, 5)]
        else:
        ...
        if current_level == 1:
            ...
            prev_level_req = xp_requirements[current_level - 1]
        else:
            prev_level_req = 3000 + (current_level - 6) * 500
        
        # Calculate progress
        xp_for_current_level = current_level_xp - prev_level_req
        xp_needed_for_next = next_level_xp_req - prev_level_req
        
        if xp_needed_for_next > 0:
            progress = xp_for_current_level / xp_needed_for_next
            progress = max(0, min(1, progress))
        else:
            progress = 1.0
        
        st.markdown("#### 📈 Progress to Level " + str(current_level + 1))
        st.progress(progress, text=f"{xp_for_current_level}/{xp_needed_for_next} XP")
...
def main():
        ...
        if user_data['completed_lessons'] or user_data['quiz_scores']:
            ...
            if activity_data:
                ...
                fig = px.bar(
                    ...
                    color_discrete_map={'Lesson': '#2563eb', 'Quiz': '#7c3aed'}
                )
    ...
    <div class="app-footer">
        <h3>🚀 CrossFi Quest</h3>
        <p>Gamified blockchain education platform</p>
        <p><strong>Learn • Earn • Build the future of decentralized finance</strong></p>
        <p style="margin-top: 1rem; opacity: 0.8;">
            🌐 <a href="https://crossfi.org" style="color: #06b6d4;">CrossFi.org</a> • 
            📚 <a href="https://docs.crossfi.org" style="color: #06b6d4;">Documentation</a> • 
            💬 <a href="https://discord.gg/crossfi" style="color: #06b6d4;">Community</a>
        </p>
    </div>
    """, unsafe_allow_html=True)
